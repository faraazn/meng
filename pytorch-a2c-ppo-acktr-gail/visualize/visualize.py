...
def gen_eval_vid_frame(actor_critic, env_state, x, max_x, pct, rew, t, action, obs, tgt_layer):
    actor_critic.eval()

    with torch.no_grad():
        features = list(list(actor_critic.children())[0].children())[0]
        classifier = list(actor_critic.children())[1]
        probs = classifier(features(obs)).probs[0].detach().cpu().numpy()

    score_cam = ScoreCam(features, classifier, target_layer=tgt_layer)
    cam = score_cam.generate_cam(obs, action)

    obs_array = np.uint8(obs[0].detach().cpu().numpy()).transpose((1,2,0))  # shape [224, 320, 3]
    ...
    draw.text(
        ...
        f"""    {env_state} | t: {t}
             ...
             action: {ACTIONS[action]} | probs: {probs}""",
        (255,255,255))
